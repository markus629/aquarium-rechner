// =============================================================
// Gemeinsame Auth + Cloud-Sync für alle Rechner (PocketBase)
// =============================================================
...
// ---------- PocketBase (singleton aus pb-client.js) ----------
// pb.authStore hält den Login-Zustand und persistiert ihn in localStorage.

// Anzeigename eines Nutzer-Records
function displayNameOf(u) { return (u && (u.name || u.email)) || "?"; }
    ...
    const render = () => {
      ...
      overlay.innerHTML = `
        <div class="as-modal" style="position:relative">
          ...
          ${!isReset ? `
            ...
            <input type="password" id="as-pass" autocomplete="${isSignup ? "new-password" : "current-password"}" placeholder="${isSignup ? "mind. 8 Zeichen" : ""}">
          ` : ""}
      ...
      submit.onclick = async () => {
        ...
        try {
          ...
          let msg = (e && e.message) || "Fehler";
          if (mode === "login") {
            msg = "E-Mail oder Passwort falsch.";
          } else if (mode === "signup") {
            if (data.email) {
              msg = data.email.code === "validation_not_unique"
                ? "Diese E-Mail ist bereits registriert."
                : "E-Mail-Adresse ungültig.";
            } else if (data.password) {
              msg = "Passwort zu schwach (mind. 8 Zeichen).";
            } else {
              msg = "Registrierung fehlgeschlagen.";
            }
          }
          if (/Failed to fetch|NetworkError|ERR_/.test(String(e && e.message))) msg = "Keine Internetverbindung.";
          setMsg(msg, "error");
          submit.disabled = false;
        }
      };
...
function buildUserMenu(btn, user, onSync, onLogout) {
  ...
  menu.innerHTML = `
    <div class="as-menu-info">
      ...
      ${!isVerified ? `<div class="as-sync-status" style="color:#d09020">⚠️ E-Mail nicht verifiziert</div>` : ""}
    </div>
...
export async function initAuthSync(opts) {
  ...
  // ---------- Save / Load (Collection "calculators": 1 Doc pro Rechner) ----------
  async function saveCloud() {
    ...
    try {
      ...
      let rec = null;
      try {
        rec = await pb.collection("calculators").getFirstListItem(`user="${uid}" && key="${calculatorId}"`);
      } catch (_) { /* 404 = noch nicht vorhanden */ }
      if (rec) {
        await pb.collection("calculators").update(rec.id, { data: state });
      } else {
        await pb.collection("calculators").create({ user: uid, key: calculatorId, data: state });
      }
    } catch (e) {
  ...
  async function loadCloud() {
    ...
    try {
      const uid = pb.authStore.record.id;
      const rec = await pb.collection("calculators").getFirstListItem(`user="${uid}" && key="${calculatorId}"`);
      return rec.data || null;
    } catch (e) {
      return null; // 404 = keine Cloud-Daten
    }
  }
  ...
  async function handleLogin(user, isNew, sentVerification) {
    ...
    if (loginOnly) {
      toast(isNew ? "Konto erstellt." : "Angemeldet als " + displayNameOf(user), "success");
      return;
    }
    // Cloud-Daten holen
  ...
  function updateBtn() {
    if (currentUser) {
      const initials = displayNameOf(currentUser).trim()[0].toUpperCase();
      btn.innerHTML = `<span class="as-avatar">${initials}</span><span class="as-btn-label" style="max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${currentUser.name || currentUser.email.split("@")[0]}</span>`;
